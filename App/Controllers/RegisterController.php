...
class RegisterController
{
    ...
    public function verifyData()
    {
        ...
        if (empty($this->email) || empty($this->user_username)) {
            echo json_encode(["error" => "Faîtes votre sélection !!!"]);
        } else {
            ...
            if ($result) {
                ...
                if(count($res) > 0)
                {
                    echo json_encode(["error" => "L'email entré existe déjà !!!"]);
                } else {
                    if(strlen($this->user_username) >= 4)
                    {
                        $resultat = $this->usermodel->verifyName($this->user_username);
                        if(count($resultat) > 0)
                        {
                            echo json_encode(["error" => "Le pseudo entré existe déjà !!!"]);
                        } else {
                            echo json_encode(["result" => "good"]);
                        }
                    } else {
                        echo json_encode(["error" => "Votre pseudo doit comportér un certain nombre de caractères supérieurs à 4 !!!"]);
                    }
                }
